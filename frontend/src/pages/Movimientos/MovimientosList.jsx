...
/**
 * MovimientosList: Componente para mostrar una lista de movimientos de equipos en una tabla.
 * Carga los datos de movimientos desde el backend.
 */
const MovimientosList = () => {
  ...
  if (loading) return <Loader message="Cargando movimientos..." className="mt-8" />;
  if (error) return <p className="text-center text-red-500 mt-8">{error}</p>;

  return (
    <Card>
      <h3 className="text-xl font-semibold text-gray-800 mb-4">Lista de Movimientos</h3>
      <Table headers={headers} data={movements} renderRow={renderRow} />
    </Card>
  );
};
